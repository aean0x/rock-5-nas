...
stdenv.mkDerivation {
  ...
  installPhase = ''
    ...
    sleep 1
    # Root partition (partition 1 on target device if different from boot device)
    ...
    nixos-generate-config --root /mnt || { echo "Failed to generate hardware configuration"; exit 1; }

    # Copy hardware configuration to repository
    echo "Copying hardware configuration to repository..."
    cp /mnt/etc/nixos/hardware-configuration.nix /tmp/nixos-config/hosts/system/hardware-configuration.nix || { echo "Failed to copy hardware configuration"; exit 1; }
